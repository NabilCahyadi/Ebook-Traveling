...
class UserActivityLogController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!class_exists('App\\Models\\ActionLog')) {
            return view('admin.user-management.activity-logs.index', [
                'logs' => collect([]),
                'users' => collect([]),
        ...
        return view('admin.user-management.activity-logs.index', compact('logs', 'users', 'actions'));
    }
    ...
    public function show($id)
    {
        ...
        return view('admin.user-management.activity-logs.show', compact('log'));
    }
    ...
    public function bulkDelete(Request $request)
    {
        $request->validate([
            ...
            'ids.*' => 'exists:action_logs,id',
        ]);

        try {
            ActionLog::whereIn('id', $request->ids)->delete();

            return redirect()->route('admin.user-activity-logs.index')
                ->with('success', __('admin.activity_logs.logs_deleted_successfully') . " (" . count($request->ids) . " logs)");
        } catch (\Exception $e) {
            return back()->with('error', 'Failed to delete activity logs: ' . $e->getMessage());
        }
    }
}
