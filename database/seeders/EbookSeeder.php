...
class EbookSeeder extends Seeder
{
    ...
    { // 1. Cek apakah data master sudah ada
        ...
        // 2. Ambil semua ID dari cities dan categories untuk efisiensi
        ...
        $ebooks = [
            ...
            [
                ...
                'author' => 'Tim Transportasi Nusantara',
        ...
        foreach ($ebooks as $data) {
            // Cari ID kota dan kategori berdasarkan slug
            ...
            // Siapkan data untuk tabel ebooks
            $ebookId = Str::uuid();
            $ebooksToInsert[] = [
                'id' => $ebookId,
                'title' => $data['title'],
                ...
                'city_id' => $cityId, // Diisi dengan ID kota
                'status' => $data['status'],
            ...
            // Siapkan data untuk tabel pivot ebook_categories
            foreach ($categoryIds as $categoryId) {
                $pivotDataToInsert[] = [
                    'id' => Str::uuid()->toString(), // <-- TAMBAHKAN BARIS INI
                    'ebook_id' => $ebookId,
        ...
        // 4. Masukkan data ke tabel ebooks dan ebook_categories
        if (!empty($ebooksToInsert)) {
            DB::table('ebooks')->insert($ebooksToInsert);
            $this->command->info('✅ ' . count($ebooksToInsert) . ' e-books created successfully!');
        }

        if (!empty($pivotDataToInsert)) {
            DB::table('ebook_categories')->insert($pivotDataToInsert);
            $this->command->info('✅ Ebook-Category relationships created successfully!');
        }
    }
}
