            ...
            <div class="card-header">
                    ...
                    <div class="col-md-12">
                        <div class="d-flex gap-2">
                            ...
                            <select class="form-select form-select-sm" id="perPageSelect" onchange="changePerPage()"
                                style="width: 130px;">
                                <option value="10" {{ request('per_page', 10) == 10 ? 'selected' : '' }}>{{ __('admin.common.per_page_items', ['count' => 10]) }}</option>
                                <option value="20" {{ request('per_page', 10) == 20 ? 'selected' : '' }}>{{ __('admin.common.per_page_items', ['count' => 20]) }}</option>
                                <option value="30" {{ request('per_page', 10) == 30 ? 'selected' : '' }}>{{ __('admin.common.per_page_items', ['count' => 30]) }}</option>
                                <option value="50" {{ request('per_page', 10) == 50 ? 'selected' : '' }}>{{ __('admin.common.per_page_items', ['count' => 50]) }}</option>
                                <option value="100" {{ request('per_page', 10) == 100 ? 'selected' : '' }}>{{ __('admin.common.per_page_items', ['count' => 100]) }}</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end mt-3 mx-3">
                    <button type="button" class="btn btn-sm btn-outline-secondary" id="toggleBulkMode"
                        ...
                        <i class="ti ti-checkbox me-1"></i> {{ __('admin.common.select_multiple') }}
                    </button>
                </div>
            </div>

            <!-- Bulk Actions Bar -->
            <div id="bulkActionsBar" class="mx-3 mt-3 mb-3 p-3 bg-light rounded d-none">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center">
                        <span class="me-3"><strong id="selectedCount">0</strong> {{ __('admin.common.items_selected') }}</span>
                    </div>
                    <div class="d-flex gap-2 flex-wrap">
                        ...
                        <div class="dropdown">
                            <button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button"
                                ...
                                <i class="ti ti-switch-horizontal me-1"></i> {{ __('admin.common.change_status') }}
                            </button>
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="#" onclick="bulkChangeStatus('draft')"><i
                                            class="ti ti-pencil me-2 text-warning"></i>{{ __('admin.ebooks.draft') }}</a></li>
                                <li><a class="dropdown-item" href="#" onclick="bulkChangeStatus('published')"><i
                                            class="ti ti-check me-2 text-success"></i>{{ __('admin.ebooks.published') }}</a></li>
                                <li><a class="dropdown-item" href="#" onclick="bulkChangeStatus('scheduled')"><i
                                            class="ti ti-clock me-2 text-info"></i>{{ __('admin.common.scheduled') }}</a></li>
                                <li><a class="dropdown-item" href="#" onclick="bulkChangeStatus('unpublished')"><i
                                            class="ti ti-eye-off me-2 text-secondary"></i>{{ __('admin.common.unpublished') }}</a></li>
                            </ul>
                        </div>
                        <!-- Delete Button -->
                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="bulkDelete()">
                            <i class="ti ti-trash me-1"></i> {{ __('admin.common.move_to_trash') }}
                        </button>
                    </div>
                </div>
            </div>
                                        ...
                                        <div class="dropdown d-inline-block">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <a class="dropdown-item text-danger" href="javascript:void(0);"
                                                    onclick="confirmDelete({{ $ebook->id }}, '{{ addslashes($ebook->title) }}')">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start mb-2">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <a class="dropdown-item text-danger" href="javascript:void(0);"
                                                    onclick="confirmDelete({{ $ebook->id }}, '{{ addslashes($ebook->title) }}', 'card')">
                                                    <i class="ti ti-trash me-2"></i> {{ __('admin.actions.delete') }}
                                                </a>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-auto pt-2">
                                        <div class="d-flex justify-content-between align-items-center mb-2 gap-2">
                                            <span class="badge bg-label-info flex-shrink-0">
                                                <i class="ti ti-file me-1"></i>{{ $ebook->page_count ?? 0 }} {{ __('admin.common.pages') }}
                                            </span>
                                            @if ($ebook->status === 'published')
                                                <span class="badge bg-success">{{ __('admin.ebooks.published') }}</span>
                                            @elseif($ebook->status === 'draft')
                                                <span class="badge bg-warning">{{ __('admin.ebooks.draft') }}</span>
                                            @elseif($ebook->status === 'scheduled')
                                                <span class="badge bg-info"><i class="ti ti-clock me-1"></i>{{ __('admin.common.scheduled') }}</span>
                                            ...
                                            @elseif($ebook->status === 'unpublished')
                                                <span class="badge bg-secondary">{{ __('admin.common.unpublished') }}</span>
                                        ...
                                        <div class="small text-muted d-flex align-items-center">
                                            ...
                                            <span>{{ number_format($ebook->view_count ?? 0) }} {{ __('admin.common.views') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    @push('scripts')
        <script>
            ...
            window.bulkDelete = function () {
                ...
                Swal.fire({
                    title: `Move ${ids.length} Ebook(s) to Trash?`,
                    html: `<p class="text-muted">Are you sure you want to move ${ids.length} selected ebook(s) to trash?</p>`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '<i class="ti ti-trash me-1"></i> Yes, Move to Trash',
            ...
            window.confirmDelete = function(ebookId, ebookTitle, viewType = 'table') {
                const formId = viewType === 'card' ? `delete-ebook-card-${ebookId}` : `delete-ebook-${ebookId}`;
                
                Swal.fire({
                    title: 'Move to Trash?',
                    html: `<p class="text-muted">Are you sure you want to move <strong>${ebookTitle}</strong> to trash?</p>`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '<i class="ti ti-trash me-1"></i> Yes, Move to Trash',
                    cancelButtonText: '<i class="ti ti-x me-1"></i> Cancel',
